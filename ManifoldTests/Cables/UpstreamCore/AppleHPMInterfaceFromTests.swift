...
/// Covers `AppleHPMInterface.from(...)` -- the pure factory the watcher uses to turn a
/// raw IOKit property dictionary into a `AppleHPMInterface`. Fixture dictionaries are
...
/// machines arrive.
@Suite("AppleHPMInterface.from factory")
struct USBCPortFromTests {
    ...
    func m2MBA_USBCPort_Parses() throws {
        let port = try #require(AppleHPMInterface.from(
            entryID: 0x1000005c4,
    ...
    func connectionActivePreservedFromProperties() throws {
        let disconnected = try #require(AppleHPMInterface.from(
        ...
        let connected = try #require(AppleHPMInterface.from(
    ...
    func hpmControllerUUIDDistinguishesMagSafeFromUSBC() throws {
        ...
        let magSafe = try #require(AppleHPMInterface.from(
            entryID: 10, serviceName: "Port-MagSafe 3@1",
            className: "AppleTCControllerType11",
            read: { self.m2MBA_MagSafe_Connected[$0] },
            hpmControllerUUID: magSafeUUID
        ))
        let usbC = try #require(AppleHPMInterface.from(
