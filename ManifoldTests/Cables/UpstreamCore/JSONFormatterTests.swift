...
struct JSONFormatterTests {
    ...
    private func makePort(connected: Bool = true) -> AppleHPMInterface {
        AppleHPMInterface(
            id: 1,
    ...
    func adapterDTOMinimalFields() throws {
        let adapter = CableAdapterInfo(watts: 30, isCharging: nil, source: "AC")
        let json = try JSONFormatter.render(
            ports: [], sources: [], identities: [], showRaw: false,
            adapter: adapter
        )
    ...
    func adapterDTOHVCMenuOmittedWhenEmpty() throws {
        let adapter = CableAdapterInfo(watts: 30, isCharging: nil, source: "AC", hvcMenu: [])
        let json = try JSONFormatter.render(
            ports: [], sources: [], identities: [], showRaw: false,
            adapter: adapter
        )
