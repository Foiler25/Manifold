...
public struct PortSummary {
    ...
    public let linkSpeed: CableLinkSpeed?

    public init(status: Status, headline: String, subtitle: String, bullets: [String], linkSpeed: CableLinkSpeed? = nil) {
        self.status = status
...
private func resolveLinkSpeed(
    ...
    if hasTB {
        ...
        if total >= 80 {
            return CableLinkSpeed(tier: .tb80, badge: "80G")
        }
        return CableLinkSpeed(tier: .tb40, badge: "40G")
    }
    if hasUSB3 {
        switch usb3Gbps(port: port, devices: devices, transports: usb3Transports) {
        case 20: return CableLinkSpeed(tier: .usb20g, badge: "20G")
        case 10: return CableLinkSpeed(tier: .usb10g, badge: "10G")
        default: return CableLinkSpeed(tier: .usb5g, badge: "5G")  // SuperSpeed floor
        }
    }
    if hasUSB2 {
        return CableLinkSpeed(tier: .usb2, badge: "480M")
    }
    return nil
}
